...
$A.UploadList = Ext.extend($A.Component,{
    ...
    onUpdate : function(ds,record, name, value){
        ...
        if (Ext.isEmpty(percent) || status == 1) {
            ...
            this.wrap.child("#atm_"+record.id).child('.j').update('<a target="_self" href="'+this.downloadurl+'?attachment_id='+record.get('attachment_id')+'&table_name='+record.get('table_name')+'&table_pk_value='+record.get('table_pk_value')+'&token='+this.token+'\" title="'+record.get("file_name")+'">'+record.get('file_name')+'</a>');
    ...
    drawList: function(){
        ...
        Ext.each(ds.getAll(),function(record){
            ...
            html.push('<div class="up_card" id="atm_'+record.id+'">');
            html.push('<div class="icon icon-'+type+'"> </div>');
            html.push('<div class="j"><a target="_self" href="'+this.downloadurl+'?attachment_id='+record.get('attachment_id')+'&table_name='+record.get('table_name')+'&table_pk_value='+record.get('table_pk_value')+'&token='+this.token+'\" title="'+record.get("file_name")+'">'+record.get('file_name')+'</a></div>');
