...
Chart.prototype = {
			...
			for(var j=0;j<this.yAxis.length;j++){
				...
				for(var k=0,l=yAxisNames.length;k<l;k++){
					...
					for(var key in group){
						this.addSeries(group[key],false);
					}
				}
	    	}
	...
	firstRender: function () {
		...
		if ($A.RangeSelector && options.rangeSelector.enabled) {
			chart.rangeSelector = new $A.RangeSelector(chart);
		}
		...
		if ($A.Scroller && (options.navigator.enabled || options.scrollbar.enabled)) {
			chart.scroller = new $A.Scroller(chart);
		}
