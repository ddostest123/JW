...
var _N = '',
    ...
    $STRIP = '.'+STRIP,
    ERROR = '销毁Tab出错: ',
    STRIP_TPL=['<div class="strip unactive" unselectable="on" onselectstart="return false;">', '<div style="width:{stripwidth}px;" class="strip-center"><div class="tab-prompt">{prompt}</div><div class="tab-close"></div></div>'],
    BODY_TPL='<div style="width:{bodywidth}px;height:{bodyheight}px;left:-10000px;top:-10000px;" class="tab"></div>';
	
	var NEW_STRIP_TPL=['<div class="strip unactive" unselectable="on" onselectstart="return false;">', '<div class="strip-center"><div class="tab-prompt" title="{prompt}">{prompt}</div><div class="tab-close"><i class="icon-remove"></i></div></div>'];
	var NEW_BODY_TPL = '<div style="width:{bodywidth}px;height:{bodyheight}px;left:-10000px;top:-10000px;" class="tab main-tab-content"></div>';
/**
 * @class Aurora.Tab
...
//      this.intervalIds=[];
    ...
    processListener: function(ou){
        ...
        sf.script[ou]('dblclick',sf.ondblclick, sf);
    },
    initEvents:function(){
    ...
    openTab : function(ref,prompt){
        ...
        if(sf.fireEvent(EVT_BEFORE_OPEN,sf,l)!==false){
            ...
            var head = sf.head,body = sf.body,script = sf.script, strip = head.child('.strip-center'),
                stripwidth=Math.max(A.TextMetrics.measure(strip||document.body,prompt).width+45,sf.scriptwidth),                
...
//      var bodys = Ext.DomQuery.select('div.tab',this.body.dom);
    ...
    onClick : function(e,t){
        var el=Ext.fly(t);
        if(el.hasClass(tc) || el.hasClass('icon-remove'))this.closeTab(el.parent($STRIP));
    },
    ondblclick : function(e, t){
    	var el=Ext.fly(t);
    	var d = el.dom;
    	if (d.className.indexOf('strip-center') < 0){
    		d = d.parentNode;
    	}
    	if (d.className.indexOf('strip-center') > -1){
    		if (d.children.length > 1){
    			this.closeTab(el.parent($STRIP));
    		}
    	}
    },
...
//      var bodys = Ext.DomQuery.select('div.tab',this.body.dom);
	...
	openMainTab : function (url, prompt, turnUrl){
	 	var sf = this,i=0,
	     items = sf.items,l = items.length;
	    for(;i<l;i++){
	        var item = items[i];
	    ...
	    if(sf.fireEvent(EVT_BEFORE_OPEN,sf,l)!==false){
	    	var iframeSign = 'iframe_' + (window.iframeSign++) + '_id';
	    	ref = turnUrl + '?url=' + escape(jDawn.addUrlAgrs(url, 'iframeSign', iframeSign)) + '&iframeSign=' + escape(iframeSign);
	        items.push({'ref':ref,prompt:prompt, url:url});
	        var head = sf.head,body = sf.body,script = sf.script, strip = head.child('.strip-center'),
	            stripwidth=Math.max(A.TextMetrics.measure(strip||document.body,prompt).width+45,sf.scriptwidth),                
	            width = head.getWidth()+stripwidth+6;
	        head.setWidth(width);
	        ...
	        new Ext.Template(NEW_STRIP_TPL).append(head.dom,{'prompt':prompt,'stripwidth':stripwidth,'stripwidth2':stripwidth+6});
	        new Ext.Template(NEW_BODY_TPL).append(body.dom,{'bodywidth':body.getWidth(),'bodyheight':body.getHeight()});
	        sf.selectTab(l);
	    }
	}
});
