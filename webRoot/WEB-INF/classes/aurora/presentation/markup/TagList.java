	...
	public String toString(String version) {
		...
		while (it.hasNext()) {
			...
			if (obj != null) {
				if (n > 0 && endLine)
					buf.append('\n');
				if (obj instanceof ScriptReference)
					buf.append(((ScriptReference) obj).toString(version));
				else if (obj instanceof StyleSheetReference) {
					buf.append(((StyleSheetReference) obj).toString(version));
				}
				n++;
			}
		}
